...
export default function TagsPage(){
    ...
    const load=async()=>{ const r = await API.get("/tags"); setRows(pickData(r)??[]); };
    useEffect(()=>{ load(); },[]);
    const create=async()=>{ await API.post("/tags",{ name }); setName(""); load(); };
    const update=async(id:number, patch:any)=>{ await API.put(`/tags/${id}`, patch); load(); };
    const del=async(id:number)=>{ await API.delete(`/tags/${id}`); load(); };
    ...
    return (
        <div className="space-y-4">
            ...
            <table className="w-full bg-white border">
                <thead><tr className="bg-gray-50 text-left"><th className="p-2">ID</th><th>Ad</th><th>Aktif</th><th></th></tr></thead>
                <tbody>{rows.map(r=>(
